...
```{r Supplemental Figure 3a, fig.align='center', fig.width=6, fig.height=6}

# Samples not included in the paper
aerosol_direct_samples = sample.metadata.df %>% 
  filter(Experiment %in% c(19)) %>% 
  pull(Run) %>% 
  unique()
...
replicates.unfiltered.df = variant.df %>%
  # Remove the isolate that isn't sequenced in duplicate. 
  filter(Run != "HL-2162") %>% 
  filter(!Run %in% aerosol_direct_samples) %>% 
  # Unique run condition
  mutate(Identifier = paste(Animal, DPI, sep="_"),
         Replicate = paste("Replicate", Replicate, sep="_")
         ) %>% 
...
ggsave(paste(figures.dir, "supplemental-figre-3a.png", sep=""), width = 6, height = 6, dpi = 300)

```

```{r Supplemental Figure 3b, fig.align='center', fig.width=6, fig.height=6}
...
variant.df %>%
  ...
  mutate(Label= paste(Animal, " ", DPI, "-DPI", sep="")) %>% 
  # Plot the difference
  ggplot(aes(x = RNA_Copies, y = Mean_Delta_AF, label = Label)) + 
    geom_point() + 
    xlab("Viral RNA Copies") + 
    ylab("Mean Allele Frequency Difference") + 
    theme_bw() + 
...
ggsave(paste(figures.dir, "supplemental-figre-3b.png", sep=""), width = 6, height = 6, dpi = 300)

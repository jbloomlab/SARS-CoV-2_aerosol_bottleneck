...
```{r Supplemental Figure 4, fig.align='center', fig.width=12, fig.height=6}

merged.df %>% 
  filter(!is.na(Pair)) %>% 
  filter(!SNP %in% fixed.isolate.variants) %>% 
  mutate(Pair = if_else(Pair == 'contact', 'recipient', 'donor')) %>% 
  ggplot(aes(x = AF, fill = factor(Pair, levels = c("donor", "recipient")))) + 
    geom_histogram(bins = 30) +
    facet_wrap(~factor(Pair, levels = c("donor", "recipient")), ncol=2) + 
    xlab("Allele Frequency") + 
    ylab("Allele Count") + 
    scale_fill_manual(values = c("darkblue", "darkred"), name="") + 
    theme_bw(18) + 
...
ggsave(paste(figures.dir, "supplemental-figure-6.png", sep=""), width = 12, height = 6, dpi = 300)

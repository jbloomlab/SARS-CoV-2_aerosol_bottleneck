...
```{r Plot Raw Bottlenecks, echo=T, fig.align='center', fig.width=8, fig.height=4}
...
ggsave("./unfiltered_bottleneck.png", width = 8, height = 4)
...
```{r Raw TV Plots, echo=T, fig.align='center', fig.width=8, fig.height=8}
...
ggsave("./unfiltered_tv_plot.png", width = 8, height = 8)
...
```{r Shared Variants, echo=T, fig.width=25, fig.height=20, fig.align="center"}
...
ggsave("./unfiltered_shared_variant_plot.png", width = 25, height = 20)
...
```{r Plot Filtered Bottlenecks, echo=T, fig.align='center', fig.width=8, fig.height=4}
...
ggsave("./filtered_bottleneck.png", width = 8, height = 4)

```

Generally, the relationship holds, which is pretty interesting. Let's see what the 'TV' plots look like. These will give us some sense of whether we got rid of variants that could be false-positives. 

```{r Filtered TV Plots, echo=T, fig.align='center', fig.width=8, fig.height=8}
...
ggsave("./filtered_tv_plot.png", width = 8, height = 8)

```

Let's take a closer look at the shared variants that are left over.

```{r Filtered Shared Variants, echo=T, fig.width=25, fig.height=20, fig.align="center"}
...
ggsave("./filtered_shared_variant_plot.png", width = 25, height = 20)
...
```{r Stock Contact Bottleneck, echo=TRUE, warning=FALSE}

# Make a data frame of isolate/contact pairs 
stock.comparison.df = merged.df %>% 
  filter(Pair == "donor") %>% 
...
Now, let's see what the bottleneck looks like between the stock and each donor animal

```{r Plot Stock Bottleneck, echo=TRUE, fig.align='center', fig.width=8, fig.height=4}
...
ggsave("./stock_bottleneck.png", width = 8, height = 4)


```

There is some variability in the bottleneck estimate, but less than I might have expected. Let's see if this improves when we use the filtered set of variants. 

```{r Plot Filtered Stock Bottleneck, echo = T, warning=FALSE}

# Make a data frame of isolate/contact pairs 
stock.comparison.df = filtered.merged.df %>% 
  filter(Pair == "donor") %>% 
  select(Experiment, contact = "Animal")  %>% 
  distinct() %>% 
  mutate(donor = "stock")
...
for (i in 1:nrow(stock.comparison.df)) {
  ...
  contact = stock.comparison.df[i,]$contact
  # Get the earliest day for each condition
  earliest.donor.dpc = filtered.merged.df %>% 
    filter(Animal == donor) %>% 
    pull(DPC) %>% 
    min()
  earliest.contact.dpc = filtered.merged.df %>% 
    filter(Animal == contact) %>% 
    pull(DPC) %>% 
    min()
  # Add these to the combinations
...
ggsave("./filtered_stock_bottleneck.png", width = 8, height = 4)
...
Generally, the conclusion of this notebook so far is that the bottleneck is narrow, but it still influenced by possible false positive variants. It's not totally clear what the impact of particle size is on the bottleneck. Generally it seems like the bottleneck isn't strong influenced. 

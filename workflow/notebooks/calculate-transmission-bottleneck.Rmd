...
text.y.position = max(bottleneck.df$bottleneck) + (.1 * max(bottleneck.df$bottleneck))
...
bottleneck.df %>% 
  ...
  ggplot(aes(x = Condition, y = bottleneck, col = Condition)) + 
    ...
    geom_text(data = mean.bottleneck, aes(x = Condition, y = text.y.position, label = round(bottleneck, 2)), size = 4.5, col = "black") + 
    geom_point(data = mean.bottleneck, shape=1, size = 3.5, col = "black") + 
...
# Position of the mean bottleneck label
text.y.position = max(filtered.bottleneck.df$bottleneck) + (.1 * max(filtered.bottleneck.df$bottleneck))
...
filtered.bottleneck.df %>% 
  ...
  mutate(Pair = paste(donor, contact, sep = "-")) %>%  
  ggplot(aes(x = Condition, y = bottleneck, col = Condition)) + 
    geom_text(data = mean.filtered.bottleneck, aes(x = Condition, y = text.y.position, label = round(bottleneck, 2)), size = 4.5, col = "black") + 
    geom_point(data = mean.filtered.bottleneck, shape=1, size = 3.5, col = "black") + 
    geom_jitter(width = 0.1, size = 1.5) + 
    xlab("") + 
    ylab("Bottleneck Size") + 
    theme_bw(16)  +
...
This makes sense if these are some kind of sequencing related artifact or contamination. Let's see what happens if we filter out variants that are only in a single sequencing round. 
...
ggsave(paste(figures.dir, "supplemental_figure_aerosol_v_direct_bottleneck.png", sep=""), width = 6, height = 4)
...
ggsave(paste(figures.dir, "supplemental_figure_impactor_bottleneck.png", sep=""), width = 8, height = 4)
...
# Calculate the mean bottleneck for each condition
mean.multiround.bottleneck = multi.round.bottleneck.df %>% 
  left_join(., conditions, by = c("contact" = "Animal")) %>% 
  mutate(Condition = fct_relevel(as_factor(Condition), condition.order)) %>% 
  mutate(Pair = paste(donor, contact, sep = "-")) %>% 
  group_by(Condition) %>% 
...
# Position of the mean bottleneck label
text.y.position = max(multi.round.bottleneck.df$bottleneck) + (.1 * max(multi.round.bottleneck.df$bottleneck))
...
multi.round.bottleneck.df %>% 
  ...
  mutate(Pair = paste(donor, contact, sep = "-")) %>%  
  ggplot(aes(x = Condition, y = bottleneck, col = Condition)) + 
    geom_text(data = mean.multiround.bottleneck, aes(x = Condition, y = text.y.position, label = round(bottleneck, 2)), size = 4.5, col = "black") + 
    geom_point(data = mean.multiround.bottleneck, shape=1, size = 3.5, col = "black") + 
    geom_jitter(width = 0.1, size = 1.5) + 
    xlab("") + 
    ylab("Bottleneck Size") + 
    theme_bw(16)  +
...
```{r Separated Summary Plot, fig.align='center', fig.width=8, fig.height=4}
...
aerosol.experiments = merged.df %>% 
  select(Condition, Experiment) %>% 
  filter(Condition %in% c('aerosol transmission', 'direct transmission')) %>% 
  pull(Experiment)

# Split the bottleneck data frame
impactor.multi.round.bottleneck.df = multi.round.bottleneck.df %>% filter(experiment %in% impactor.experiments)
aerosol.multi.round.bottleneck.df = multi.round.bottleneck.df %>% filter(experiment %in% aerosol.experiments)

# Calculate the mean bottleneck for each condition
mean.impactor.bottleneck = mean.multiround.bottleneck %>% 
  filter(Condition %in% c('no impactor', '5 um impactor', '2.5 um impactor'))
mean.aerosol.bottleneck = mean.multiround.bottleneck %>% 
  filter(Condition %in% c('aerosol transmission', 'direct transmission'))

# Position of the mean bottleneck label
impactor.text.y.position = max(impactor.multi.round.bottleneck.df$bottleneck) + (.1 * max(impactor.multi.round.bottleneck.df$bottleneck))
aerosol.text.y.position = max(aerosol.multi.round.bottleneck.df$bottleneck) + (.1 * max(aerosol.multi.round.bottleneck.df$bottleneck))

# Add the conditions to the data frame and plot 
impactor.multi.round.bottleneck.df %>% 
  left_join(., conditions, by = c("contact" = "Animal")) %>% 
  mutate(Condition = fct_relevel(as_factor(Condition), condition.order)) %>% 
  mutate(Pair = paste(donor, contact, sep = "-")) %>%  
  ggplot(aes(x = Condition, y = bottleneck, col = Condition)) + 
    # geom_text(data = mean.impactor.bottleneck, aes(x = Condition, y = impactor.text.y.position, label = round(bottleneck, 2)), size = 4.5, col = "black") + 
...
ggsave(paste(figures.dir, "figure_impactor_summarized_bottleneck.png", sep=""), width = 6, height = 4)

# Add the conditions to the data frame and plot 
aerosol.multi.round.bottleneck.df %>% 
  left_join(., conditions, by = c("contact" = "Animal")) %>% 
  mutate(Condition = fct_relevel(as_factor(Condition), condition.order)) %>% 
  mutate(Pair = paste(donor, contact, sep = "-")) %>%  
  ggplot(aes(x = Condition, y = bottleneck, col = Condition)) + 
    # geom_text(data = mean.aerosol.bottleneck, aes(x = Condition, y = aerosol.text.y.position, label = round(bottleneck, 2)), size = 4.5, col = "black") + 
    geom_point(data = mean.aerosol.bottleneck, shape=1, size = 3.5, col = "black") + 
    geom_jitter(width = 0.1, size = 1.5) + 
    xlab("") + 
    ylab("Bottleneck Size") + 
    theme_bw(16)  +
...
ggsave(paste(figures.dir, "supplemental_figure_aerosol_v_direct_summarized_bottleneck.png", sep=""), width = 3.5, height = 4)
...
mean.multiround.bottleneck %>% 
  kable(., caption = "Average Botlteneck Sizes")

write_csv(mean.multiround.bottleneck, paste(figures.dir, "supplemental_figure_mean_bottleneck_sizes.csv", sep=""))
...
ggsave(paste(figures.dir, "supplemental_figure_stock_bottleneck.png", sep=""), width = 8, height = 4)

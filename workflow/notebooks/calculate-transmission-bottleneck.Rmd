...
```{r Plot Stock Bottleneck, fig.align='center', fig.width=8, fig.height=4}
...
ggsave(paste(figures.dir, "stock_bottleneck.png", sep=""), width = 8, height = 4)
...
# Make a data frame of isolate/contact pairs 
stock.comparison.df = masked.merged.df %>% 
  filter(Pair == "donor") %>% 
  select(Experiment, contact = "Animal")  %>% 
  distinct() %>% 
  mutate(donor = "stock")
...
for (i in 1:nrow(stock.comparison.df)) {
  ...
  contact = stock.comparison.df[i,]$contact
  # Get the earliest day for each condition
  earliest.donor.dpc = masked.merged.df %>% 
    filter(Animal == donor) %>% 
    pull(DPC) %>% 
    min()
  earliest.contact.dpc = masked.merged.df %>% 
    filter(Animal == contact) %>% 
    pull(DPC) %>% 
    min()
  # Add these to the combinations
...
Interestingly, these variants that have the appearance of false-positive variants are only found in the first round of sequencing. None are found in the second round. 
...
def upload_via_ftp(tar_path,
    ...
    with ftplib.FTP(ftp_address) as ftp:
        ...
        if ftp_subfolder not in ftp.nlst():
...
if __name__ == '__main__':
    ...
    assert args.sampletype in {'illumina'}

    if args.sampletype == 'illumina': 

        print("\nUploading runs for Illumina Runs.\n")
        
        print(f"ftp_subfolder: {config['ftp_subfolder']}")

        upload_via_ftp(tar_path="SRA_submission.tar",
        ftp_username=config['ftp_username'],
        ftp_account_folder=config['ftp_account_folder'],
